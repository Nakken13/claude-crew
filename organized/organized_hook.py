...
À chaque fin de tour :
  1. régénère organized/<dir>/INDEX.md (titres + liens),
  2. journalise les transitions (démarrée / terminée / ajoutée) dans
     organized/CLAUDE_CONTEXT/CHANGELOG_TACHES.md,
...
DIRS = {
    "PROBLEMS": ORGANIZED / "PROBLEMS",
    "TODO": ORGANIZED / "TODO",
    "ICEBOX": ORGANIZED / "ICEBOX",
    "CURRENT_TASKS": ORGANIZED / "CURRENT_TASKS",
    "TESTS": ORGANIZED / "TESTS",
    "TESTS/IA": ORGANIZED / "TESTS" / "IA",
    "TESTS/DEV": ORGANIZED / "TESTS" / "DEV",
}
CTX = ORGANIZED / "CLAUDE_CONTEXT"
SNAP = CTX / ".task_state.json"
CHANGELOG = CTX / "CHANGELOG_TACHES.md"
BATCH_FILE = ORGANIZED / "CLAUDE_BATCH.md"

INTRO = {
    "PROBLEMS": "Problèmes. Résolu → déplacer le contexte vers `HISTORIQUE.md`.\n\n",
    "TODO": "Tâches pas commencées. Démarrer = déplacer le fichier vers `organized/CURRENT_TASKS/` (cf. `CLAUDE.md`).\n\n",
    "ICEBOX": "Idées/tâches parkées volontairement (distinct de TODO). Pour reprendre : déplacer vers `organized/TODO/` d'abord.\n\n",
    "CURRENT_TASKS": "Tâches en cours. Finie → supprimer + entrée `organized/CLAUDE_CONTEXT/HISTORIQUE.md` + `organized/TESTS/<chantier>.md`.\n\n",
...
def active_task_slugs():
    """Slugs (fichiers .md) actuellement en organized/TODO/ ou organized/CURRENT_TASKS/."""
    active = set()
    for d in (DIRS["TODO"], DIRS["CURRENT_TASKS"]):
        if d.exists():
...
def main():
    ...
    if finished:  # rappel non bloquant tracé au changelog
        entries.append(f"  ↳ ⚠️ vérifier : entrée dans `HISTORIQUE.md` + checklist `organized/TESTS/<chantier>.md` "
    ...
    if entries and prev:  # ne pas journaliser le premier snapshot de référence
        ...
        if not CHANGELOG.exists():
            head = ("# Changelog des tâches\n\n"
                    "> Alimenté automatiquement par le hook Stop (`organized/organized_hook.py`).\n\n")
    ...
    if dup:
        reason = ("Incohérence cycle de vie : tâche(s) présente(s) à la fois dans "
                  "organized/TODO/ et organized/CURRENT_TASKS/ : " + ", ".join(sorted(dup)) +
                  ". Retire-les de organized/TODO/ (une tâche commencée ne reste pas dans le backlog).")
