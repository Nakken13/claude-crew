...
→ crée automatiquement la tâche associée dans organized/CURRENT_TASKS/<slug>.md.
...
CURRENT = ROOT / "organized" / "CURRENT_TASKS"
TODO = ROOT / "organized" / "TODO"
...
""",
    ...
    print(
        json.dumps(
            {
                "systemMessage": f"Spec → tâche créée : organized/CURRENT_TASKS/{slug}.md",
                "hookSpecificOutput": {
                    "hookEventName": "PostToolUse",
                    "additionalContext": (
                        f"Une tâche organized/CURRENT_TASKS/{slug}.md vient d'être créée automatiquement "
                        f"depuis la spec {spec_rel}. Complète sa section Actions avec les étapes concrètes "
                        "de la spec, et référence la tâche dans organized/CLAUDE_BATCH.md (règle de batching)."
                    ),
                },
            }
        )
